...
=begin
  P
  -the computer will check for an immediate threat
  -an immediate threat is defined as two squares marked by opponent in a row
  -the computer will pick at random if there is no immediate threat

  A
  -define immediate threat as 2 squares marked by opponent in a row
    -identify the immediate threat by checking all of the sub_arrays in the winning_lines nested array
      -check each index to see if there is a PLAYER_MARKER
      -if there are two squares (sub_array elements) with PLAYER_MARKER in a row
        -the computer will choose the remaining empty element in that array
      -if the above condition is false
        -the computer will choose a random square to play by referencing the compute_places_piece method 
  

...reference to this method for the computer play...
