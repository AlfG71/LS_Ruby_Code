...
-it will sort the array in place from lowest to highest value (mutates the argument)

Rules:
-the argument will contain at least two elements
-it will stop iterating once every number value is less than the value that preceeds it
-strings can be included in the argument
...
2) iterate over the argument until each number or word is less than the number or word that follows it
   -from the first index until the last index of the array
     -compare if the index is greater than index + 1
       -if greater
       	-swap index + 1 for index
       -else
        -compare the next two numbers
     -stop iteration once it iterates over the argument and nothing is swapped

3) return the swapped array        
=end
